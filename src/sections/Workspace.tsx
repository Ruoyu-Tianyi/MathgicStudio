...
import {
  ...
  Database, Cog, Info, FileText, Sparkles, FileUp,
} from 'lucide-react'
import {
...
export default function Workspace({ contest, onContestChange }: Props) {
  ...
  return (
    <section id="workspace" className="mx-auto max-w-6xl scroll-mt-20 px-4 py-20">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-slate-900">在线体验</h2>
        <p className="mt-3 text-slate-600">粘贴赛题文本或直接上传 PDF，体验赛道判定与五阶段流水线，生成专属论文大纲。</p>
      </div>

      <Alert className="mx-auto mt-6 max-w-3xl border-indigo-200 bg-indigo-50/60">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg">① 输入赛题</CardTitle>
            ...
            <CardDescription>当前配置：{c.fullName} · 论文语言 {c.language}</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">② 流水线执行</CardTitle>
            <CardDescription>P0 审题 → P1 数据 → P2 建模 → P3 写作 → P4 交付，每阶段过门禁后推进。</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div
              ref={logRef}
              className="h-[240px] overflow-y-auto rounded-lg border border-slate-200 bg-slate-900 p-4 font-mono text-xs leading-relaxed"
            >
              {logLines.length === 0 ? (
